...
MACD Histogram and 5% Stochastic Indicator Trading
...
Potential Changes:
Stochastic Indicator Percent Changes
Time Frame Changes
MACD Period Changes
Additional Filters
...
def trader(context, data):
    ...
    for stock in context.results.index:
        ...
        highs= data.history(stock, 'high', 50, '1d')
        lows= data.history(stock, 'low', 50, '1d')
        slowk, slowd = stochastic(highs, lows, prices)
        kvalue= slowk[-1]
        dvalue= slowd[-1]
        ...
        elif macd > 0 and position == 0 and macdline > 0 and cash > price and leverage <1 and kvalue > 0.95*dvalue and kvalue < 1.05*dvalue:
